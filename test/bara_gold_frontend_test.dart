...
class _GoldBilling extends BillingController {
  ...
  @override
  Future<BillingResult> buyCoins(CoinPackOffer pack) async => const BillingResult(success: true, message: 'ok');
  @override
  ...
  @override
  Future<BillingResult> startTrial(BillingPlan plan) async => const BillingResult(success: true, message: 'ok');
  @override
  Future<BillingResult> subscribe(BillingPlan plan) async => const BillingResult(success: true, message: 'ok');
  @override
  Future<BillingResult> restore() async => const BillingResult(success: true, message: 'ok');
  @override
  Future<BillingResult> cancelRenewal() async => const BillingResult(success: true, message: 'ok');
  @override
  Future<BillingRerollResult> reroll({required String raceId, required List<String> ids,
    required RerollFunding funding}) async => const BillingRerollResult(success: true, message: 'ok');
}

void main() {
  setUp(() {
    PackageInfo.setMockInitialValues(appName: 'Bara', packageName: 'test', version: '1.0',
  ...
  testWidgets('Bara Gold paywall only presents current weekly/monthly benefits', (tester) async {
    final billing = _GoldBilling();
    await tester.pumpWidget(BillingScope(controller: billing,
  ...
  testWidgets('Gold character policy renders framed label and direct-IAP fallback', (tester) async {
    final character = ShopCharacter.fromJson({
      'characterKey': 'mouse', 'name': 'Mouse',
      'item': {'id': 'mouse', 'sku': 'mouse', 'slot': 'CHARACTER', 'assetKey': 'mouse', 'priceCoins': 1000},
      'owned': false, 'canPurchase': false, 'goldAccess': true, 'coinPurchaseAllowed': false,
      'directPurchase': {'available': true, 'storeProductId': 'bara_character_mouse_v1'},
      'unavailableReason': 'requires_gold_or_direct_purchase',
    });
    await tester.pumpWidget(MaterialApp(home: Scaffold(body: ShopCharacterCard(character: character))));
  ...
  testWidgets('Gold character with missing IAP metadata offers Get Gold', (tester) async {
    final character = ShopCharacter.fromJson({
      'characterKey': 'mouse', 'name': 'Mouse',
      'item': {'id': 'mouse', 'slot': 'CHARACTER', 'assetKey': 'mouse', 'priceCoins': 1000},
      'owned': false, 'canPurchase': false, 'goldAccess': true,
      'directPurchase': {'available': true},
    });
    var opened = false;
    await tester.pumpWidget(MaterialApp(home: Scaffold(body: ShopCharacterCard(
      character: character, onGetGold: () => opened = true))));
  ...
  testWidgets('direct character purchase action uses the server product ID', (tester) async {
    final billing = _GoldBilling();
    final character = ShopCharacter.fromJson({
      'characterKey': 'sea_lion', 'name': 'Sea Lion',
      'item': {'id': 'sea_lion', 'sku': 'sea_lion', 'slot': 'CHARACTER', 'assetKey': 'sea_lion', 'priceCoins': 1000},
      'owned': false, 'canPurchase': false, 'goldAccess': true, 'coinPurchaseAllowed': false,
      'directPurchase': {'available': true, 'storeProductId': 'bara_character_sea_lion_v1'},
    });
    await tester.pumpWidget(MaterialApp(home: Scaffold(body: ShopCharacterCard(
      character: character,
      onDirectBuy: () => billing.buyDirectProduct(character.directStoreProductId!),
    ))));
