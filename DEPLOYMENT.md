...
Follow [README's native Meta setup](README.md#native-meta-ios-configuration).
Supply `--dart-define=META_APP_ID=1600882908142439` and
`--dart-define="META_CLIENT_TOKEN=${META_CLIENT_TOKEN:?Set the Meta client token}"`
on iOS commands. The normal Xcode build decodes only those two values into a
derived native Info.plist; no Python setup command or generated local xcconfig
is needed. Keep the actual token in local/CI configuration, not committed files
or logs. Never substitute a Meta app secret or access token.

Release/Profile validate the defines automatically; missing/invalid Debug values
leave measurement disabled safely. Direct Xcode builds need Flutter build settings
generated by a configured Flutter command first. Android omits the Meta defines.
Mediation, OAuth, RevenueCat and retained AdMob arguments remain unchanged.
Automatic events and advertiser-ID collection stay disabled in plist defaults.

The upload command verifies the archived app's actual plist against the intended
values, without printing the token. A configured tree does not verify an older
archive. Avoid shell tracing and raw Xcode build-settings dumps.
...
To run against PROD instead, use the complete
[production iPhone command in README](README.md#run-on-a-physical-iphone-against-prod).
It includes the required RevenueCat key, all seven retained AdMob units and
the production OAuth client alongside the production backend URL.
...
META_ARCHIVED_PLIST=build/ios/archive/Runner.xcarchive/Products/Applications/Runner.app/Info.plist
# Capture for comparison only: never print the embedded client token.
test "$(plutil -extract FacebookAppID raw -o - "$META_ARCHIVED_PLIST")" = 1600882908142439 && \
test "$(plutil -extract FacebookClientToken raw -o - "$META_ARCHIVED_PLIST")" = "${META_CLIENT_TOKEN:?Set the Meta client token}" && \
test "$(plutil -extract FacebookAutoLogAppEventsEnabled raw -o - "$META_ARCHIVED_PLIST")" = false && \
test "$(plutil -extract FacebookAdvertiserIDCollectionEnabled raw -o - "$META_ARCHIVED_PLIST")" = false && \
