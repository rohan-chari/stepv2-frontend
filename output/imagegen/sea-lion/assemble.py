...
s.save(p/'sea_lion_walk_right.png');frames[0].save(p/'sea_lion_thumb.png')
pre=[]
for f in frames:
 bg=Image.new('RGBA',f.size,'white');bg.alpha_composite(f);pre.append(bg.convert('RGB').resize((384,384),Image.Resampling.NEAREST))
pre[0].save(p/'sea_lion-walk-preview.gif',save_all=True,append_images=pre[1:],duration=120,loop=0)
