...
for i in range(6):
 ...
 c=c.crop(c.getbbox())
 c=c.resize((round(c.width*.205),round(c.height*.205)),Image.Resampling.NEAREST)
...
s.save(p/'hedgehog_walk_right.png');frames[0].save(p/'hedgehog_thumb.png')
pre=[]
for f in frames:
 bg=Image.new('RGBA',f.size,'white');bg.alpha_composite(f);pre.append(bg.convert('RGB').resize((384,384),Image.Resampling.NEAREST))
pre[0].save(p/'hedgehog-walk-preview.gif',save_all=True,append_images=pre[1:],duration=120,loop=0)
