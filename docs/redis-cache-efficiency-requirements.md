...
Status: deployed to production on September10,2026 after explicit user approval of the documented unrelated baseline-test exceptions. Writer A cb790cd preceded reader B 488f8b1480ed33d370fca118eaca36946b17466b. Redis200mb and production health verified. No frontend release is required.
...
## Implementation audit amendments (2026-09-10)

Architect-reviewed catalog correction: extended Home presentation caches store reusable identity and equipped item references, not mutable catalog fields. Every read, including a Redis hit, hydrates catalog presentation through one bounded batched PostgreSQL query with the existing capability/channel filters. Peer mirroring and raw catalog repair have no target Redis invalidation connection; source-process invalidation cannot protect the other database. This retains equipment/topology savings without claiming the catalog lookup disappears. The additive presentation payload uses a random `ce:v1:g:presentation:<user>` marker; A writers preserve the existing numeric cosmetics generation and additionally advance this marker. No peer Redis configuration, feature flag or database migration is introduced.

Operational review maintenance uses the same awaited post-commit consistency model as API writers. Managed seed/reset wrappers capture affected users (including ordinary friendship counterparts), race participants/races and local dates before deletion and after writes, retain a private recovery manifest before commit, and invalidate each committed phase before the next phase or successful exit. Bounded failures exit nonzero with an executable invalidation-only replay command. The manifest binds the PostgreSQL target, Redis target and namespace; a mismatched replay must retain it and refuse work. Raw SQL requires a wrapper-supplied session execution marker. This is an invocation guard, not a release flag. Abrupt termination between commit and invalidation has the same residual window as the approved A API protocol; do not claim atomic PostgreSQL/Redis commits. The wrapper preserves the reviewer login row as its original contract intended and explicitly clears milestone claims during reset.

Freshness audit: existing loaded eligible events exclude future starts and pending local entitlements. Widening the shared score freshness window therefore requires a separate, additive internal boundary proof. Its design must be anchored to score `asOf` and generation fences captured before scoring inputs are read, cover race/effect/event boundaries including those crossed before publication, and distinguish verified absence from failed/missing proof. Hard boundary validity applies independently of paged age tolerance. One bounded publication-time aggregate may be justified only with total worker/request work measurements. This slice remains gated on the follow-up architect review and boundary integration evidence; it must not silently rely on publication time or current viewer banner data.


Implementation verification update: the boundary proof is implemented with five pre-input generation tokens, original scoredAt retained through durable tasks, one bounded publication boundary query, canonical effect phase boundaries and atomic publication fencing. The dedicated public HTTP/PostgreSQL/Redis/separate-worker suite passed20/20; paged age tolerance and the existing15s queue convergence check remain separate. The final full backend unit suite passed3391/3391. Independent code review cleared the batch-fence, payout recovery merge, malformed-payload fallback and metrics fixes. Complete measurements and the outstanding burst/final release evidence live in backend `docs/redis-cache-efficiency-validation.md`. Broader pre-existing regression failures are explicitly recorded there and have not been silently changed or skipped.

Final bounded burst:2000 successful uploads,12m durable and resolved steps,four committed queue generations,zero remaining triggers/post-tasks. The burst exposed and verified a minimal pre-existing queue promoter fix in B. Detailed actual A/B query/Redis measurements are in backend `docs/redis-cache-efficiency-reader-evidence.md`. No application deployment has occurred.

Deployment evidence: backend `docs/redis-cache-efficiency-production-deploy-2026-09-10.md`. Both guarded cutovers completed; two HTTP workers retained, staging stopped, API/Redis healthy.
