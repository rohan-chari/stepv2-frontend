...
const EXPERIMENT_PROFILE_NAME='app4-ram8-disk160-workers5-pool8-adaptive250';
...
function configuredExperiment(){
  ...
  return Object.freeze({
    name:EXPERIMENT_PROFILE_NAME,label:'EXPERIMENTAL ONLY — adaptive run beginning at 250 RPS',
    app:Object.freeze({cpus:4,memoryGiB:8,diskGiB:160}),workers:5,poolSize:8,
    workload:Object.freeze({...WORKLOAD,mode:'adaptive',startRps:250,stepRps:10}),
  });
}
...
const poolSize=(()=>{try{const dbSource=fs.readFileSync('/var/www/step-tracker-backend/src/db.js','utf8');const literal=/new pg\.Pool\(\{[\s\S]*?max:\s*(\d+)/.exec(dbSource)?.[1];if(literal)return Number(literal);if(!/new pg\.Pool\(\{[\s\S]*?max:\s*databasePoolMax\b/.test(dbSource))return 0;const safetySource=fs.readFileSync('/var/www/step-tracker-backend/src/localCapacitySafety.js','utf8');return Number(/const productionDefault\s*=\s*(\d+)\s*;/.exec(safetySource)?.[1]||0)}catch{return null}})();
